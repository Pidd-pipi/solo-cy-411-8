...
- **多实例并发**（`multi-instance.test.js`）：同一旧库/空库上同时启动两个实例时，只有一个 leader 执行结构变更，另一个 waiter 在咨询锁上等待并复用结果；两实例都健康、结构只建一次、`schema_migrations` 只登记一条。存量冲突时并发的两个实例都明确拒绝启动且保数据（不能因互相重复建列而失败），修复后并发重启全部成功。
- **迁移故障注入**（`migration-failover.test.js`，均用独立真实连接，非内存替身、非单连接串行）：
  - 独立连接长期占用迁移锁超过 `MIGRATION_LOCK_TIMEOUT`，等待实例在等待时限后以**非零状态退出、不监听端口、不做任何 DDL**；释放锁后新实例正常完成升级。
  - leader 已持锁并在首个 `ADD COLUMN` 中途（用独立连接持有表元数据锁把 ALTER 卡住）被 `SIGKILL`，剩余实例拿到咨询锁后**续跑成功**，结构完整、迁移登记唯一、历史回填正确。
  - 修复重复数据后并发重启，所有实例都恢复健康。
